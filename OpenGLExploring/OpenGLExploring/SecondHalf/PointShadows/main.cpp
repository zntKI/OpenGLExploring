...
#pragma region Setup
	...
	window = glfwCreateWindow (SCREEN_WIDTH, SCREEN_HEIGHT, "Point shadows", NULL, NULL);
	if (!window) {
		glfwTerminate ();
		return -1;
	}
...
#pragma region Main
	...
	Shader depthShader ("SecondHalf/PointShadows/Shaders/depthVertexShader.glsl", "SecondHalf/PointShadows/Shaders/depthFragmentShader.glsl", "SecondHalf/PointShadows/Shaders/depthGeometryShader.glsl");

	Shader shadowShaderSimple ("SecondHalf/PointShadows/Shaders/shadowVertexShader.glsl", "SecondHalf/PointShadows/Shaders/shadowFragmentShader.glsl");
...
#pragma region FirstPass
		...
		for (unsigned int i = 0; i < 6; i++)
		{
			depthShader.setMatrix4 ("u_ShadowMatrices[" + std::to_string (i) + "]", shadowTransforms[i]);
		}
		depthShader.setVec3 ("u_LightPos", lightPos);
		depthShader.setFloat ("u_FarPlane", far_plane);
...
#pragma region SecondPass
		...
		shadowShaderSimple.setFloat ("u_FarPlane", far_plane);


		renderShadowScene (shadowShaderSimple);
...
void renderDepthSceneSimple (Shader& shader)
{
	...
	//glCullFace (GL_FRONT); // Because we render the cube from inside out
	shader.setInt ("u_ReverseNormals", 1); // A small little hack to invert normals when drawing cube from the inside so lighting still works.
	renderCube ();
	shader.setInt ("u_ReverseNormals", 0); // Switch back to normal
	//glCullFace (GL_BACK); // Switch back to normal
